...
void renderCornellboxA(float scale, const char* filename) {
    ...
    // walls
    makeWalls(walls, scale, nullptr, mat);
	walls->rebuildIndex();
	scene->add(walls);
...
void renderCornellboxB(float scale, const char* filename) {
   ...
   // makeWalls(scene, scale, nullptr, mat);
    ...
    //short box
    makeBox(scene2, Point(082.f, 000.1f, 225.f)*scale, Vector(158.f, 000.f, 047.f)*scale, Vector(048.f, 000.f, -160.f)*scale, Vector(000.f, 165.f, 000.f)*scale, nullptr, mat);

    //tall box
    makeBox(scene2, Point(265.f, 000.1f, 296.f)*scale, Vector(049.f, 000.f, 160.f)*scale, Vector(158.f, 000.f, -049.f)*scale, Vector(000.f, 330.f, 000.f)*scale, nullptr, mat);

    //point light
    world.light.push_back(new PointLight(Point(288*scale,529.99f*scale,279.5f*scale),RGBColor::rep(20000.0f*scale*scale)));
	//world.light.push_back(new JuliaLight(Point(288 * scale, 529.99f*scale, 279.5f*scale), RGBColor::rep(20000.0f*scale*scale)));
    //spot light
